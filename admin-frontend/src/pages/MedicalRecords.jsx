...
import { useGetHorsesQuery, useAddMedicalRecordMutation, useUpdateMedicalRecordMutation, useDeleteMedicalRecordMutation, useGetMedicalRecordsQuery } from '../slices/apiSlice';

const MedicalRecords = () => {
    const { data: horses = [] } = useGetHorsesQuery();
    const { data: medicalRecords = [] } = useGetMedicalRecordsQuery();
    ...
    const handleHorseSelection = (event) => {
        ...
        if (checked) {
            setSelectedHorses([...selectedHorses, value]);
        } else {
            setSelectedHorses(selectedHorses.filter(horseId => horseId !== value));
        }
    };
    ...
    const handleFormSubmit = async (event) => {
        ...
        if (editMode) {
            ...
            for (const horseId of selectedHorses) {
                await addMedicalRecord({ ...recordToSubmit, horseId });
            }
        }
        handleModalClose();
    };
                ...
                <tbody>
                    {currentRecords.map(record => (
                        <tr key={record.id}>
                            ...
                            <td>{horses.find(horse => horse.id === record.horseId)?.name || 'Unknown'}</td>
            ...
            <Modal show={showModal} onHide={handleModalClose}>
                ...
                <Modal.Body>
                    <Form onSubmit={handleFormSubmit}>
                        ...
                        {!editMode && (
                            <Form.Group controlId="formHorses">
                                <Form.Label>Select Horses</Form.Label>
                                {horses.map(horse => (
                                    <Form.Check
                                        key={horse.id}
                                        type="checkbox"
